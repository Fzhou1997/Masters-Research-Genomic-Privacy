...
class Classifier(nn.Module):
    def __init__(self,
        ...
        self.linear = nn.Linear(hidden_size * (2 if bidirectional else 1), 3)
...
def classify(predicted: torch.Tensor) -> torch.Tensor:
    return torch.argmax(torch.softmax(predicted, dim=1), dim=1).long()


def interpret(labels: torch.Tensor) -> torch.Tensor:
...
class Tester:
    def __init__(self,
        ...
        self.loss = loss
        self.accuracy = Accuracy(task='multiclass', num_classes=3).to(device)
        self.f1_score = F1Score(task='multiclass', num_classes=3).to(device)
        self.precision = Precision(task='multiclass', num_classes=3).to(device)
        self.recall = Recall(task='multiclass', num_classes=3).to(device)
        self.auroc = AUROC(task='multiclass', num_classes=3).to(device)
        self.confusion_matrix = ConfusionMatrix(task='multiclass', num_classes=3).to(device)
        self.test_loader = test_loader
