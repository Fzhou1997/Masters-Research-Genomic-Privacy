...
class PhenotypeHairColor:
    ...
    def load(self, file_name):
        ...
        dataframe.columns = map(lambda column: column.replace(' ', '_'), dataframe.columns)
        hair_color_column = dataframe['hair_color'].bfill(axis=1).iloc[:, 0]
        dataframe = dataframe.drop('hair_color', axis=1)
        dataframe['hair_color'] = hair_color_column
        dataframe['hair_color'] = dataframe['hair_color'].str.lower()
        dataframe = dataframe[dataframe.apply(lambda row: 'exome-vcf' not in row['genotype_filename'], axis=1)]
        dataframe = dataframe[dataframe.apply(lambda row: 'IYG' not in row['genotype_filename'], axis=1)]
        dataframe = dataframe[['user_id', 'hair_color']]
