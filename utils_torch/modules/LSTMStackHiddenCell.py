...
class LSTMStackHiddenCell(nn.Module):
    ...
    def __init__(self,
        ...
        super(LSTMStackHiddenCell, self).__init__()
        ...
        for i in range(num_layers):
            self._lstm_modules.append(LSTMLayerHiddenCell(input_size=input_sizes[i],
    ...
    def forward(self, x: Tensor, hx: tuple[tuple[Tensor, Tensor], ...] = None) -> tuple[tuple[tuple[Tensor, Tensor], tuple[Tensor, Tensor]], ...]:
        x_i = x
        if hx is None:
        ...
        for i, lstm in enumerate(self._lstm_modules):
            hx_i = hx[i]
            (x_i, c_i), hx_i = lstm.forward(x_i, hx_i)
            outputs.append(((x_i, c_i), hx_i))
            if i >= self._inner_num_layers:
                continue
