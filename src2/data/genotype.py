...
class GenotypeProcessor:
    def __init__(self, raw_path: str, res_path: str, out_path: str, build: int):
        ...
        self.genotype_combinations = list(itertools.product(self.alleles, repeat=2))
    ...
    def load_raw(self, user_id: int) -> DataFrame:
        ...
        if not user_filenames:
            raise FileNotFoundError(f'No genotype files found for user {user_id}')
        snps = None
        ...
        if snps is None:
            raise ValueError(f'No valid genotype files found for user {user_id}')
        snps.sort()
        genotype_df = snps.snps
        genotype_df = genotype_df[genotype_df['chrom'].isin(self.chromosomes)]
        genotype_df['genotype'] = genotype_df['genotype'].apply(lambda x: '--' if '-' in x else x)
        for combination in self.genotype_combinations:
            genotype_df[combination] = (genotype_df['genotype'] == combination).astype(int)
        return genotype_df
    ...
    def load_processed(self, user_id: int) -> DataFrame:
        ...
        if user_filename not in self.processed_filenames:
            raise FileNotFoundError(f'No processed genotype file found for user {user_id}')
    ...
    def impute(self, genotype_df: DataFrame, hair_color: str) -> DataFrame:
        ...
        for rsid, row in genotype_df.iterrows():
            if row['genotype'] == '--':
                genotype_imputed = random.choice(a=self.genotype_combinations, p=self.genotype_distribution[hair_color][rsid])
